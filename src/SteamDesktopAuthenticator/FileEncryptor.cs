...
public static class FileEncryptor
{
    ...
    public static string DecryptData(string password, string passwordSalt, string IV, string encryptedData)
    {
        ...
        var key = GetEncryptionKey(password, passwordSalt);
        ...
        using (var aes256 = new RijndaelManaged())
        {
            ...
            var decryptor = aes256.CreateDecryptor(aes256.Key, aes256.IV);
    ...
    public static string EncryptData(string password, string passwordSalt, string IV, string plaintext)
    {
        ...
        using (var aes256 = new RijndaelManaged())
        {
            ...
            var encryptor = aes256.CreateEncryptor(aes256.Key, aes256.IV);

            using (var msEncrypt = new MemoryStream())
            {
                using (var csEncrypt = new CryptoStream(msEncrypt, encryptor, CryptoStreamMode.Write))
                {
                    using (var swEncypt = new StreamWriter(csEncrypt))
                    {
