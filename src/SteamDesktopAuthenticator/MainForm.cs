...
public partial class MainForm : Form
{
    ...
    private void btnTradeConfirmations_Click(object sender, EventArgs e)
    {
        ...
        var oText = btnTradeConfirmations.Text;
    ...
    private void btnManageEncryption_Click(object sender, EventArgs e)
    {
        if (manifest.Encrypted)
        {
            ...
            var curPassKey = currentPassKeyForm.txtBox.Text;
            ...
            var newPassKey = changePassKeyForm.txtBox.Text;
            var confirmPassKey = changePassKeyForm2.txtBox.Text;
            ...
            var action = newPassKey == null ? "remove" : "change";
    ...
    private void menuRemoveAccountFromManifest_Click(object sender, EventArgs e)
    {
        if (manifest.Encrypted)
        {
            MessageBox.Show("You cannot remove accounts from the manifest file while it is encrypted.",
                "Remove from manifest", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        else
        {
            var res = MessageBox.Show(
                ...
                "Remove from manifest", MessageBoxButtons.OKCancel);
    ...
    private async void menuDeactivateAuthenticator_Click(object sender, EventArgs e)
    {
        ...
        if (currentAccount.Session.IsRefreshTokenExpired())
        {
            MessageBox.Show("Your session has expired. Use the login again button under the selected account menu.",
                "Deactivate Authenticator", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }
        ...
        if (currentAccount.Session.IsAccessTokenExpired())
        {
            ...
            {
                MessageBox.Show(ex.Message, "Deactivate Authenticator Error", MessageBoxButtons.OK,
        ...
        var res = MessageBox.Show(
            ...
            "Deactivate Authenticator: " + currentAccount.AccountName, MessageBoxButtons.YesNoCancel);
        var scheme = 0;
        ...
        if (scheme != 0)
        {
            var confCode = currentAccount.GenerateSteamGuardCode();
            var confirmationDialog = new InputForm(string.Format(
                "Removing Steam Guard from {0}. Enter this confirmation code: {1}", currentAccount.AccountName,
                confCode));
            ...
            var enteredCode = confirmationDialog.txtBox.Text.ToUpper();
            ...
            var success = await currentAccount.DeactivateAuthenticator(scheme);
            if (success)
            {
    ...
    {
        var names = new List<string>(getAllNames());
        names = names.FindAll(IsFilter);
    ...
    private async void timerTradesPopup_Tick(object sender, EventArgs e)
    {
        ...
        var accs =
            manifest.CheckAllAccounts ? allAccounts : new[] { currentAccount };
        ...
        try
        {
            ...
            foreach (var acc in accs)
            {
                ...
                {
                    MessageBox.Show(
                        "Your session for account " + acc.AccountName +
                        " has expired. You will be prompted to login again.", "Trade Confirmations",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                try
                {
                    var tmp = await acc.FetchConfirmationsAsync();
    ...
    private void listAccounts_KeyDown(object sender, KeyEventArgs e)
    {
        if (e.Control)
        {
            ...
            {
                var to = listAccounts.SelectedIndex - (e.KeyCode == Keys.Up ? 1 : -1);
    ...
    {
        var itemArray = new string[allAccounts.Length];
    ...
    private void checkForUpdates()
    {
        ...
        updateClient.DownloadStringAsync(
            new Uri("https://api.github.com/repos/Jessecar96/SteamDesktopAuthenticator/releases/latest"));
    }
    ...
    private void compareVersions()
    {
        if (newVersion > currentVersion)
        {
            ...
            var updateDialog = MessageBox.Show(
                string.Format(
                    "A new version is available! Would you like to download it now?\nYou will update from version {0} to {1}",
                    Application.ProductVersion, newVersion.ToString()), "New Version", MessageBoxButtons.YesNo);
    ...
    {
        var totButtons = panelButtons.Controls.OfType<Button>().Count();
